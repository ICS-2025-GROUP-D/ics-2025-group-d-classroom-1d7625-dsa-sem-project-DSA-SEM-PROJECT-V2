...
def create_music_player(parent, x=0, y=0):
    ...
    music_queue = [{"title":"Shape of You", "artist": "Ed Sheeran"},
    ...
    title_var=ctk.StringVar(value=music_queue[current_index[0]]["title"])
    artist_var=ctk.StringVar(value=music_queue[current_index[0]]["artist"])
    ...
    def update_song():
        song=music_queue[current_index[0]]
        title_var.set(song["title"])
        artist_var.set(song["artist"])

    def play_previous():
        current_index[0] = (current_index[0] - 1) % len(music_queue)
        update_song()
